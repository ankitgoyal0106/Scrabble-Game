...
const Database = async (dbname) => {
  ...
  const obj = {
    ...
    saveGameScore: async (name, score) => {
      ...
      try {
        const db = getDB();
        const data = await db.get("games"); // Retrieve the games collection
        data.games.push({ name, score }); // Add the new score
        await db.put(data); // Save the updated games collection back to the database
    ...
    top10WordScores: async () => {
      ...
      // sorting the scores from highest to lowest
        const topScores = data.words
          .sort((a, b) => b.score - a.score)
          .slice(0, 10);

        await db.close(); 
        return { status: "success", data: topScores }; 
      } catch (e) {
          return {
            status: "error",
    ...
    top10GameScores: async () => {
      ...
      try {
        ...
        // Sort the scores in descending order and slice the top 10
        const topScores = data.games
          .sort((a, b) => b.score - a.score)
          .slice(0, 10);
