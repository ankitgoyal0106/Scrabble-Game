...
async function basicServer(request, response) {
  ...
  if (method === "POST" && pathname === "/wordScore") {
    ...
    const result = await database.saveWordScore(name, word, score);
    ...
    response.end(JSON.stringify({ status: "success", data: result.data }));
    ...
    // Add your implementation here.
    const result = await database.saveGameScore(query.name, query.score);
    response.writeHead(200);
    response.end();
    // HTTP status code 501 Not Implemented is returned by a server when the
    // request method is not supported by the server and cannot be handled. This
